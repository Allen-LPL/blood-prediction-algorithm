...
def train_one_group(group, group_blood_df, weather_df):
    ...
    # 6.递推
    new_df = df.copy()
    dates = df_final_valid[DATE_COL].unique().tolist()
    final_pre = []
    for cur_date in dates:
        cur_df = new_df[new_df[DATE_COL] == cur_date]
        x_valid = cur_df[feat_cols].values
        tmp_pred = model.predict(x_valid)
        cur_pre_num = normalize_pred(tmp_pred)[0]
        final_pre.append({'date': cur_date, '总血量': cur_pre_num})
        new_df.loc[new_df[DATE_COL] == cur_date, TARGET_COL] = cur_pre_num

        new_df = add_prev_rolling_sums(new_df, windows=(1, 3, 7, 14, 28))
        new_df = add_dynamic_feature(new_df)

    pred_final_valid = pd.DataFrame(final_pre)
    ...
    plt.plot(df_plot[DATE_COL], df_plot[TARGET_COL], label="真实值")
    plt.plot(df_test[DATE_COL], pred_test, label="测试集预测")
    plt.plot(df_final_valid[DATE_COL], pred_final_valid[TARGET_COL], label=f"最终验证集预测(最后{final_valid_days}天)")
    ...
    plt.plot(df_final_valid[DATE_COL], df_final_valid[TARGET_COL].values, marker="o", label="真实值")
    plt.plot(df_final_valid[DATE_COL], pred_final_valid[TARGET_COL].values, marker="o", label="预测值(XGB)")
