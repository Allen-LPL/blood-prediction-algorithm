...
TREND_WIN = 7  # trend窗口（rolling时生效）
TREND_MODE = "ewm"  # "rolling"(滞后更明显) / "ewm"(更灵敏，推荐)
TREND_SPAN = 7  # ewm span（TREND_MODE="ewm"时生效）
...
HOLIDAY_BOOST = 0.6   # 节假日/前后一天权重提升倍数（以 t+1 为准）
EXTREME_Q = 0.90      # 极值分位阈值（0.90 表示 top/bottom 10%）
EXTREME_NEG_BOOST = 0.9  # 低谷（大负残差）加权
EXTREME_POS_BOOST = 0.25  # 尖峰（大正残差）加权（偏小，避免尖峰过冲）
WEIGHT_CLIP = (0.5, 3.0)

# 预测 log_resid 的裁剪：防止少数天残差爆炸导致尖峰
PRED_LOG_RESID_CLIP_Q = (0.001, 0.99)  # 更放开低谷，减少“谷底慢”   # 按训练残差分位裁剪预测

# XGB early-stopping（时间序列，训练末尾作为验证集）
XGB_USE_EARLY_STOP = True
XGB_EARLY_STOP_ROUNDS = 120
XGB_TRAIN_VAL_RATIO = 0.90   # 训练集前 90% 训练，后 10% 验证

# XGB 外生特征对齐策略：
# - "none": 不引入 t+1 特征
# - "calendar_only": 只为“日历/节假日/周期”生成 t+1 特征（推荐，稳定且线上可用）
# - "calendar_and_weather": 额外为温度/天气也生成 t+1 特征（离线回测可用，线上需天气预报）
SHIFT_MODE = "calendar_only"

CALENDAR_COLS = [
    "weekday_sin", "weekday_cos", "month_sin", "month_cos",
    "weekday", "month", "day", "dayofyear", "weekofyear",
    "is_weekend", "is_holiday", "is_workday", "is_before_holiday", "is_after_holiday",
    "days_until_holiday", "days_since_holiday", "near_holiday7"
]
TEMP_COLS = ["平均温度", "temp_sq", "temp_diff", "temp_rolling3"]
...
SHIFT_CALENDAR_TARGET_COLS = [
    "day_of_week",   # 周几（目标日）
    "is_weekend",    # 是否周末（目标日）
    "month",         # 月份（目标日）
    "season",        # 季节（目标日）
    "is_holiday",    # 是否节假日（目标日）
]

# 可选: "mae" / "mse" / "huber"（默认推荐 huber，能更好抗尖峰，谷底也不至于被极端点拖慢）
LSTM_LOSS = "huber"
...
def train_one_group(g: pd.DataFrame, group_name: str, feature_cols_lstm: list, feature_cols_xgb: list):
    ...
    # trend/residual（用“过去信息”构造基线，预测 y(t+1) 时不泄露未来）
    if TREND_MODE == "rolling":
        g["trend"] = g[TARGET_COL].rolling(TREND_WIN, min_periods=TREND_WIN).mean().shift(1)
    else:
        g["trend"] = g[TARGET_COL].ewm(span=TREND_SPAN, adjust=False, min_periods=TREND_SPAN).mean().shift(1)

    # 补齐开头 NaN：只向前填充（不使用未来），开头用首日值兜底
    first_val = float(g[TARGET_COL].iloc[0])
    g["trend"] = g["trend"].ffill().fillna(first_val)
    ...
    g2["lag3"] = g2[TARGET_COL].shift(3)
    g2["diff1"] = g2[TARGET_COL] - g2["lag1"]
    g2["diff3"] = g2[TARGET_COL] - g2["lag3"]
    g2["diff1_abs"] = g2["diff1"].abs()
    ...
    # 这些特征也喂给 XGB
    extra_xgb_feats = ["lag1", "lag7", "lag14", "lag28", "absdiff1", "absdiff7", "diff7", "diff14",
                      "rolling7", "rolling14", "rolling28", "rolling_std7", "rolling_std14", "rolling_absdiff7",
                      "ewm7", "ewm14", "diff1", "diff3", "diff1_abs"]
    for c in extra_xgb_feats:
        if c not in feature_cols_xgb:
            feature_cols_xgb.append(c)


    # ===== 可选：为部分外生特征生成 t+1 版本（用于预测目标那天）=====
    # lag/rolling 等历史特征仍然使用 t 时刻值（预测日之前可观测）
    lag_like = set(["lag1", "lag7", "lag14", "lag28", "rolling7", "rolling14", "rolling28", "rolling30",
                    "absdiff1", "absdiff7", "diff7", "diff14", "ewm7", "ewm14",
                    "rolling_std7", "rolling_std14", "rolling_absdiff7"])
    ...
    if SHIFT_MODE != "none":
        for c in feature_cols_xgb:
            ...
            if c in CALENDAR_COLS:
                # 仅对“目标日自身”的日历字段做 t+1 shift；
                # 像 is_before_holiday/days_until_holiday 这类本身就包含“前瞻/回溯”含义，
                # 再 shift(-1) 往往会让模型更滞后。
                if c in SHIFT_CALENDAR_TARGET_COLS:
                    shift_candidates.append(c)
            elif SHIFT_MODE == "calendar_and_weather":
                if (c in TEMP_COLS) or c.startswith("天气_"):
                    shift_candidates.append(c)
    ...
    # XGB 同时使用原始(t)特征 + t+1 特征（让模型自行决定用哪个更有用）
    feature_cols_xgb_used = list(feature_cols_xgb)
    for c in shift_candidates:
        tc = f"tplus1_{c}"
        if tc in g2.columns:
            feature_cols_xgb_used.append(tc)
    feature_cols_xgb_used = list(dict.fromkeys(feature_cols_xgb_used))
    ...
    # 目标日(t+1)距离节假日的天数（用于“窗口期”样本加权）
    for dc in ["days_until_holiday", "days_since_holiday"]:
        if dc in g2.columns:
            g2[f"tplus1_{dc}"] = g2[dc].shift(-1)
    ...
    if XGB_WEIGHT_MODE == "none":
        ...
        # 极值加权：把“低谷/尖峰”分开（你图里主要难点通常是低谷断崖）
        neg_thr = np.quantile(y_train, 1.0 - EXTREME_Q)
        pos_thr = np.quantile(y_train, EXTREME_Q)
        w_train *= (1.0 + EXTREME_NEG_BOOST * (y_train <= neg_thr).astype(float))
        w_train *= (1.0 + EXTREME_POS_BOOST * (y_train >= pos_thr).astype(float))
    ...
    if XGB_USE_EARLY_STOP and len(X_train) > 200:
        cut = int(len(X_train) * XGB_TRAIN_VAL_RATIO)
        X_tr2, X_val2 = X_train[:cut], X_train[cut:]
        y_tr2, y_val2 = y_train[:cut], y_train[cut:]
        w_tr2, w_val2 = w_train[:cut], w_train[cut:]
        xgb.fit(
            X_tr2, y_tr2,
            sample_weight=w_tr2,
            eval_set=[(X_val2, y_val2)],
            sample_weight_eval_set=[w_val2],
            verbose=False
        )
    else:
    ...
    # 裁剪预测残差，避免少数样本爆炸造成极端尖峰（按训练分位）
    ql = np.quantile(y_train, PRED_LOG_RESID_CLIP_Q[0])
    qh = np.quantile(y_train, PRED_LOG_RESID_CLIP_Q[1])
    log_resid_pred = np.clip(log_resid_pred, ql, qh)
...
def merge_calendar_weather(temp_df):
    ...
    # ===== 新增：距离节假日的天数特征 =====
    # days_to_holiday: 距离“下一个(含今天)”节假日还有几天（节假日当天=0）
    # days_since_holiday: 距离“上一个(含今天)”节假日过去几天（节假日当天=0）
    # days_from_holiday: 到“最近节假日”的有符号距离：节前为负、节后为正、节假日=0
    holiday_arr = np.sort(
        weather_df.loc[weather_df["is_holiday"] == 1, "date"].values.astype("datetime64[D]")
    )
    date_arr = weather_df["date"].values.astype("datetime64[D]")

    if holiday_arr.size == 0:
        # 没有节假日信息时，给一个“远离节假日”的常量（与 clip(0,30) 逻辑保持一致）
        days_to = np.full(len(weather_df), 30.0, dtype=float)
        days_since = np.full(len(weather_df), 30.0, dtype=float)
        days_from = np.full(len(weather_df), 30.0, dtype=float)
    else:
        # next: searchsorted(side="left")  -> 若当天是节假日，则 next==当天
        next_pos = np.searchsorted(holiday_arr, date_arr, side="left")
        # prev: searchsorted(side="right")-1 -> 若当天是节假日，则 prev==当天
        prev_pos = np.searchsorted(holiday_arr, date_arr, side="right") - 1

        has_next = next_pos < holiday_arr.size
        has_prev = prev_pos >= 0

        next_h = np.where(
            has_next, holiday_arr[next_pos.clip(0, holiday_arr.size - 1)], np.datetime64("NaT")
        )
        prev_h = np.where(
            has_prev, holiday_arr[prev_pos.clip(0, holiday_arr.size - 1)], np.datetime64("NaT")
        )

        days_to = (next_h - date_arr).astype("timedelta64[D]").astype("float")
        days_since = (date_arr - prev_h).astype("timedelta64[D]").astype("float")

        # 边界：没有前/后节假日 -> 置为 30（等价于 clip(0,30) 的“远离节假日”）
        days_to = np.where(has_next, days_to, 30.0)
        days_since = np.where(has_prev, days_since, 30.0)

        days_to = np.maximum(days_to, 0.0)
        days_since = np.maximum(days_since, 0.0)

        # 有符号距离：更靠近“下一个节假日”则为负，否则为正
        days_from = np.where(days_to <= days_since, -days_to, days_since)

    # 统一裁剪，避免过大数值影响模型（可按需调大/调小窗口）
    weather_df["days_to_holiday"] = np.clip(days_to, 0, 30)
    weather_df["days_since_holiday"] = np.clip(days_since, 0, 30)
    weather_df["days_from_holiday"] = np.clip(days_from, -30, 30)

    # 向后兼容：旧字段名
    weather_df["days_until_holiday"] = weather_df["days_to_holiday"]

    # 近节假日窗口标记
    weather_df["near_holiday7"] = (weather_df["days_from_holiday"].abs() <= 7).astype(int)
