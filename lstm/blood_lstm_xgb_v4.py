...
EXTREME_Q = 0.9            # 极值更少，更稳
EXTREME_NEG_BOOST = 0.9
EXTREME_POS_BOOST = 0.25    # 重点：降低尖峰权重
WEIGHT_CLIP = (0.5, 3.0)    # 重点：收紧上限，防止过拟合极端日

# 预测 log_resid 的裁剪：防止少数天残差爆炸导致尖峰
PRED_LOG_RESID_CLIP_Q = (0.01, 0.99)   # 按训练残差分位裁剪预测

# XGB early-stopping（时间序列，训练末尾作为验证集）
XGB_USE_EARLY_STOP = True
XGB_EARLY_STOP_ROUNDS = 120              # 更早停，泛化更稳
XGB_TRAIN_VAL_RATIO = 0.90              # 尾部验证更长一点，防“末尾过拟合”
