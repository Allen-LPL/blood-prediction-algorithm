...
def train_one_group(g: pd.DataFrame, group_name: str, feature_cols_lstm: list, feature_cols_xgb: list):
    ...
    resid_pred = xgb.predict(X_test_xgb)
    resid_true = y_test_xgb

    # ========= 合成 total =========
    L = min(len(trend_pred), len(resid_pred))
    total_pred = trend_pred[:L] + resid_pred[:L]
    ...
    print(f"[{group_name}] Resid MAPE={safe_mape(resid_true[:L], resid_pred[:L]):.4f}")

    # ========= 画图 =========
    plt.figure(figsize=(11, 4))
    plt.plot(total_true, label="real")
    plt.plot(total_pred, label="predict(LSTM+XGB)")
    plt.title(f"{group_name} total predict", fontproperties=FONT)
    plt.legend()
    plt.tight_layout()
    plt.show()
...
def main():
    ...
    for (station, blood), g in df.groupby([STATION_COL, BLOOD_COL]):
        if station == '北京市红十字血液中心' and blood in blood_types:
            group_name = f"{station}_{blood}"
            print("start train:", group_name)
            train_one_group(g, group_name, feature_cols_lstm, feature_cols_xgb)
            print("end train:", group_name)
