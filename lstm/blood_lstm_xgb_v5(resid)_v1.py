...
def make_residual_sequences(feature_mat: np.ndarray, resid_target: np.ndarray, lookback: int):
    """
...
def train_one_group(group, group_blood_df, weather_df):
    ...
    for tr_idx, va_idx in kf.split(X_train):
        ...
        xgb_fold.fit(X_tr_kf, y_tr_kf, eval_set=[(X_va_kf, y_va_kf)], sample_weight=sw_tr, verbose=0)
        oof_preds[va_idx] = xgb_fold.predict(X_va_kf)

    # 训练最终模型用于预测测试集
    xgb_final = XGBRegressor(**params)
    xgb_final.fit(X_train, Y_train, eval_set=[(X_test, Y_test)], sample_weight=sample_weight, verbose=0)
    ...
    # -------------------------
    # 挑选 LSTM 辅助特征
    lstm_aux_cols = ['y_xgb', '平均温度', 'is_spring_festival', 'is_qingming', 'is_pku_semester_start_season', 'rolling7', 'lag1', 'dayofweek', 'is_weekend', 'month']
    ...
    resid_s_target = resid_scaler.transform(df["resid"].to_numpy().reshape(-1, 1)).reshape(-1).astype(np.float32)

    X_seq, y_seq, idx_seq = make_residual_sequences(features_s, resid_s_target, lookback)
